...
class Report {
  ...
  async run (coverageReport) {
    const context = libReport.createContext({
      dir: this.reportsDirectory,
      watermarks: this.watermarks,
      coverageMap: await this.getCoverageMapFromAllCoverageFiles(coverageReport)
    })
  ...
  async getCoverageMapFromAllCoverageFiles (coverageReports) {
    ...
    const v8ProcessCov = this._getMergedProcessCov(coverageReports)
   ...
   * @private
   */
  _getMergedProcessCov (reports) {
    const { mergeProcessCovs } = require('@bcoe/v8-coverage')
    const v8ProcessCovs = []
    const fileIndex = new Set() // Set<string>
    for (const v8ProcessCov of reports) {
